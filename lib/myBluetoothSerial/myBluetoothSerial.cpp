...
static myBluetoothSerialDataCb custom_data_callback = NULL;
...
myBluetoothSerial::myBluetoothSerial()
{
    local_name = "ESP32"; //default bluetooth name
}

myBluetoothSerial::~myBluetoothSerial(void)
{
    _stop_bt();
}
...
int myBluetoothSerial::available(void)
{
    if (_spp_rx_queue == NULL)
    {
        return 0;
...
int myBluetoothSerial::peek(void)
{
    uint8_t c;
...
bool myBluetoothSerial::hasClient(void)
{
    return _spp_client > 0;
}

int myBluetoothSerial::read(void)
{
...
size_t myBluetoothSerial::write(uint8_t c)
{
    return write(&c, 1);
}

size_t myBluetoothSerial::write(const uint8_t *buffer, size_t size)
{
    if (!_spp_client)
    {
        return 0;
...
void myBluetoothSerial::end()
{
    _stop_bt();
}

esp_err_t myBluetoothSerial::register_callback(esp_spp_cb_t *callback)
{
...
void myBluetoothSerial::enableSSP()
{
    _enableSSP = true;
}
/*
     ...
     * Use fixed pin code
*/
bool myBluetoothSerial::setPin(const char *pin)
{
...
bool myBluetoothSerial::connect(uint8_t remoteAddress[])
{
    if (!isReady(true, READY_TIMEOUT))
        return false;
    if (!remoteAddress)
...
bool myBluetoothSerial::connect()
{
    if (!isReady(true, READY_TIMEOUT))
        return false;
    if (_isRemoteAddressSet)
...
bool myBluetoothSerial::disconnect()
{
    if (_spp_client)
    {
        flush();
...
bool myBluetoothSerial::unpairDevice(uint8_t remoteAddress[])
{
    if (isReady(false, READY_TIMEOUT))
    {
...
bool myBluetoothSerial::connected(int timeout)
{
    return waitForConnect(timeout);
}

bool myBluetoothSerial::isReady(bool checkMaster, int timeout)
{
